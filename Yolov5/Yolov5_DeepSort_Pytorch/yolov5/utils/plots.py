...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            area_1 = [(50, 200), (50, 400), (100, 400), (100, 200)]

            result = cv2.pointPolygonTest(np.array(area_1, np.int32), (int(cx), int(cy)), False)

            if result >= 0 and (label[0] not in sushi):
                sushi[label[0]] = time.time()
            print(sushi)

            if label[0] in sushi:
                elapsed_time = time.time() - sushi[label[0]]
                sushi_elapsed_time[label[0]] = elapsed_time
            ...
            if label:
                ...
                cv2.putText(self.im, str(label) + str(round(elapsed_time, 0)), (p1[0], p1[1] - 2 if outside else p1[1] + h + 2), 0, self.lw / 3, txt_color,
